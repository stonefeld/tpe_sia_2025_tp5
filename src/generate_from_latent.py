...
z_values = np.array([
    z_start * (1 - alpha) + z_end * alpha for alpha in alphas
])
...
for i in letras_importantes:
    ax1.annotate(chr(0x61 + i - 1), (latent_repr[i, 0], latent_repr[i, 1]), 
...
for inicio, fin, color, label in caminos:
    ...
    z_camino = np.array([
        z_inicio * (1 - alpha) + z_fin * alpha for alpha in alphas_camino
    ])
